...
function PulsingDots() {
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 8, justifyContent: 'center', marginTop: 16 }}>
      {[s1, s2, s3].map((s, i) => (
        <Animated.View key={i} style={[{ width: 10, height: 10, borderRadius: 5, backgroundColor: '#8b5cf6' }, s]} />
      ))}
    </View>
  );
}
...
export default function DashboardScreen() {
  ...
  const handleGenerateTryOn = async () => {
    ...
    try {
      ...
      formData.append('image', { uri: personImage, name: 'person.jpg', type: 'image/jpeg' } as any);
      ...
      if (data.pointsExhausted) {
        showAlert(
          ...
          [{ text: 'View Plans', onPress: () => router.push('/PaymentScreen' as any) }],
        );
      }
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (token) {
              try {
                const { data: history } = await axios.get(`${API_URL}/images/my`, {
                  headers: { Authorization: `Bearer ${token}` },
                });

                await Promise.all(
                  (history ?? []).map((record: { _id: string }) =>
                    axios
  ...
  return (
    ...
    >
      <SafeAreaView style={styles.container}>
        <Animated.View entering={FadeInDown.duration(400)} style={styles.mainWrapper}>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerRight}>
              {/* Diamond badge */}
              <TouchableOpacity onPress={() => router.push('/PaymentScreen' as any)}>
              ...
              {/* About Us */}
              <TouchableOpacity onPress={() => router.push('/AboutUsScreen' as any)} style={styles.signOutBtn}>
          ...
          {/* ── GARMENT CAROUSEL ── */}
          <Animated.View entering={FadeInDown.delay(100).duration(400)} style={styles.carouselContainer}>
            {isLoadingGarments ? (
              <View style={{ flexDirection: 'row', gap: 12, paddingHorizontal: 2 }}>
                {[0,1,2,3].map(i => (
                  <View key={i} style={[styles.garmentCard, { opacity: 0.4 + i * 0.1 }]}>
                    <View style={{ flex: 1, backgroundColor: 'rgba(139,92,246,0.15)', borderRadius: 10 }} />
                  </View>
                ))}
              </View>
            ) : (
              <FlatList
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity
                      onPress={() => !isGenerating && setSelectedGarmentId(item._id)}
          ...
          {/* ── DYNAMIC CANVAS ── */}
          <Animated.View entering={FadeIn.delay(150).duration(400)} style={styles.canvasWrapper}>
            <TouchableOpacity
              style={[
                styles.mainContainer,
                generatedImage ? styles.mainContainerSuccess : null,
              ]}
              onPress={generatedImage || isGenerating ? undefined : (personImage ? undefined : handleImageOption)}
              activeOpacity={generatedImage || personImage ? 1 : 0.7}
            >
              {isGenerating ? (
                <View style={styles.placeholder}>
                  <PulsingDots />
                  <Text style={[styles.uploadText, { marginTop: 20 }]}>Generating AI Preview...</Text>
                ...
                <View style={{ flex: 1, width: '100%' }}>
                  ...
                  {/* ✕ cancel button (#16) */}
                  <TouchableOpacity style={styles.cancelImageBtn} onPress={handleRemoveImage}>
                    <Text style={styles.cancelImageText}>✕</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.editOverlay} onPress={handleImageOption}>
                    <Text style={styles.editOverlayText}>Tap to change photo</Text>
                  </TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity style={styles.placeholder} onPress={handleImageOption} activeOpacity={0.7}>
          ...
          {/* ── ACTION BUTTON ── */}
          <Animated.View entering={FadeInUp.delay(200).duration(400)} style={styles.footer}>
            {!generatedImage ? (
              <TouchableOpacity
                style={[
      ...
      <View style={styles.footerNav}>
        <TouchableOpacity
          ...
          onPress={() => !isGenerating && router.push('/AISuggestionScreen' as any)}
          disabled={isGenerating}
        >
          <Ionicons name="sparkles-outline" size={20} color="#c4b5fd" />
          <Text style={styles.footerNavText}>AI Suggest</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={styles.footerNavBtn}
          onPress={() => !isGenerating && router.push('/TryOnHistoryScreen' as any)}
          disabled={isGenerating}
        >
          <Ionicons name="time-outline" size={20} color="#6ee7b7" />
          <Text style={styles.footerNavText}>My Try-Ons</Text>
        </TouchableOpacity>

        <TouchableOpacity
          style={styles.footerNavBtn}
          onPress={() => !isGenerating && router.push('/PaymentScreen' as any)}
          disabled={isGenerating}
        >
          <Ionicons name="diamond-outline" size={20} color="#fbbf24" />
          <Text style={styles.footerNavText}>Plans</Text>
        </TouchableOpacity>
      </View>

      {/* ── GENERATION LOCK OVERLAY (#17) ── */}
      {isGenerating && (
        <Animated.View entering={FadeIn.duration(200)} exiting={FadeOut.duration(200)} style={styles.generatingOverlay}>
          <View style={styles.generatingCard}>
            <PulsingDots />
            <Text style={styles.generatingTitle}>Generating your look...</Text>
            <Text style={styles.generatingSubtitle}>This may take up to 2 minutes</Text>
          </View>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  mainWrapper:  { flex: 1, paddingHorizontal: 20, paddingBottom: 8 },
  ...
  // Carousel
  carouselContainer: { height: 130, marginBottom: 16, justifyContent: 'center' },
  garmentCard: {
    width: 90,
    height: 130,
  ...
  footerNavText: { color: '#CBD5E1', fontSize: 11, fontWeight: '600' },
  ...
  generatingTitle:    { color: '#FFFFFF', fontSize: 18, fontWeight: '700', marginTop: 20, marginBottom: 8 },
